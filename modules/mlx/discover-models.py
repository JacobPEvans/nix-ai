...
def main() -> None:
    ...
    # physical model id, since the Nix module sets `hooks.on_startup.preload`
    # to an alias name and llama-swap resolves the alias at lookup time.
    # When that's the case, walk the aliases table to find the physical entry,
    # then update default_model so the cmd_template substitution below targets
