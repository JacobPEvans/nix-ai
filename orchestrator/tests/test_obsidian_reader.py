...
requiring llama-index-core in CI unit test environments.
"""
...
class TestExtractCallouts:
    def test_note_callout(self):
        assert "note" in _extract_callouts("> [!note]\n> This is important.")

    def test_warning_callout(self):
        assert "warning" in _extract_callouts("> [!warning]\n> Be careful here.")

    def test_multiple_callouts(self):
        callouts = _extract_callouts("> [!note]\n> Info\n\n> [!tip]\n> Helpful.")
...
class TestExtractEmbeds:
    ...
    def test_aliased_embed(self):
        assert "file.png" in _extract_embeds("![[file.png|300]]")
...
class TestMergeTags:
    def test_merge_deduplicates(self):
        result = _merge_tags(["python", "ai"], ["ai", "ml"])
        assert result == ["python", "ai", "ml"]

    def test_handles_none(self):
        assert _merge_tags(None, ["inline"]) == ["inline"]

    def test_handles_string(self):
        assert _merge_tags("single", []) == ["single"]

    def test_handles_empty(self):
        assert _merge_tags(None, []) == []
